import Color from "color";

export default class CDotRGB {
	static data() {
		return {
			name: LANG.cdotrgb.name,
			desc: LANG.cdotrgb.desc,
			s_name: "cdotrgb",
            direction_change: true,
            color_change: false,
            leds_change: true,
            rotation_change: true
		};
	}

	constructor(controller) {
		this.controller = controller;
		this.current_rgb = Color({ r: 255, g: 0, b: 0 });
		this.ids = [];
		this.direction = 1; // (1)right = 0->inf | (0)left = inf->0
		this.running = false;
		this.leds = 30;
		this.leds_count;
		this.strip = [];
		this.new_data = false;
		this.rotation = 1;
        this.class = CDotRGB;
	}

	setData(data) {
		if (data) {
			if (data.leds && Number(data.leds) != Number.NaN && Number(data.leds) > 0 && Number(data.leds) < this.controller.driver.interface.leds) {
                this.leds = Number(data.leds);
                this.head = 0;
                this.tail = this.leds;
				this.new_data = true;
            }
            if(data.rotation){
                if(Number(data.rotation) != Number.NaN && Number(data.rotation) > 0 && Number(data.rotation) < 360){
                    this.rotation = Number(data.rotation);
                }
            }
            if(data.direction){
                if(this.direction == 0){
                    this.direction = 1;
                } else{
                    this.direction = 0;
                }
            }
		}
	}

	start() {
		if (this.leds >= this.controller.driver.interface.leds) this.leds = 1;
		this.resetStrip();
		this.running = true;
		// controller.driver.allRGB({ r: 0, g: 0, b: 0 });
		// controller.driver.setRGB(this.current_location, this.current_rgb);
		this.loop();
	}

	loop() {
		if (this.running) {
			setTimeout(() => {
				this.animation();
				this.loop();
			}, (this.controller.driver.fps / 1000.0) * 100);
		}
	}

	stop() {
		this.running = false;
	}

	animation() {
		this.current_rgb.lightness(50);
		this.current_rgb.saturationl(100);

		// console.log({rgb:looper.rgb});
		// console.log({hsv});

		// console.log({hsv2:hsv});
		console.log({h: this.head, t: this.tail});

		if (this.head >= this.controller.driver.interface.leds) {
			this.direction = 0;
			this.current_rgb = this.current_rgb.rotate(this.rotation);
		} else if (this.tail < 0) {
			this.direction = 1;
			this.current_rgb = this.current_rgb.rotate(this.rotation);
		}

		if (this.direction === 0) {
			this.strip = [...this.strip.splice(1, this.strip.length), this.strip[0]];
			this.head--;
			this.tail--;
		} else {
			this.strip = [this.strip[this.strip.length - 1], ...this.strip.splice(0, this.strip.length)];
			this.head++;
			this.tail++;
		}

		if (this.running) {
			this.controller.setColors(this.strip);
		}

		if (this.newData) {
			this.new_data = false;
			this.resetStrip();
		} else {
			this.updateStrip();
		}
	}

	updateStrip() {
		for (let i = this.tail; i < this.head; i++) {
			this.strip[i] = this.current_rgb.rgb().object();
			// console.log(this.strip[i]);
		}
	}

	resetStrip() {
		for (let i = 0; i < this.controller.driver.interface.leds; i++) {
			this.strip.push({ r: 0, g: 0, b: 0 });
		}
        this.head = 0;
        this.tail = this.leds;
		if (this.tail + this.leds >= this.controller.driver.interface.leds) {
			for (let i = 0; i < this.leds; i++) {
				this.strip[this.head - i] = this.current_rgb.rgb().object();
			}
		} else {
			for (let i = this.tail; i < this.leds; i++) {
				this.strip[i] = this.current_rgb.rgb().object();
			}
		}

		this.new_data = false;
	}
}
